...
Programmiersprache wurde C gewählt, da ein kompiliertes Programm eine höhere Performanz
...
\emph{FEAL} Komponente, welche alle Funktionen besitzt, um die Funktionalität
des \emph{FEAL} Verfahrens bereitzustellen. Die nächste Komponente widmet sich
ganz der Attacke, welche von Murphy beschrieben wurde. Abschließend wird noch
...
Da Murphy in seinem Paper die meisten Funktionalitäten sowohl des \emph{FEAL},
...
32-Bit Zahl als Byte-Array der Länge 4 und behält dabei den Variablennamen
aus dem Paper. Dadurch ähnelt ein Zugriff auf den entsprechenden Index im Array
(z.B.
...
zusammen- \newline hängenden $W, V^0$ und $U^0$ Werten. Zusätzlich erleichtert
...
noch nachvoll- ziehen zu können. Wir werden an dieser Stelle nicht auf die
...
\end{lstlisting}
Auf diese Weise ist die Suche nach Fehlern in der späteren fertigen Attacke um
...
Krypto-Attacke auf das \emph{FEAL}-Verfahren durchführen kann. Im nächsten
Kapitel wird nun ein Fallbeispiel einer solchen Attacke erläutert.
